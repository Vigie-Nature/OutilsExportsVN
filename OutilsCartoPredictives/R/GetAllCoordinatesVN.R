...
for (t in 1:length(liste_fichiers)){
  ...
  if(sum(c("longitude","latitude") %in% names(Df_vn))<2){ #condition que les champs coordonnées existente
    print("latitude et/ou longitude absentes du data frame")
  }else{
    if("session_date" %in% colnames(Df_vn)){
      ...
      #(mais pour l'instant on garde pour éventuel debug d'export foireux sur les dates ou les coordonnees)
